...
//统计接口1
    zuoyetongzhiChat2() {
      this.$nextTick(()=>{
        ...
        var zuoyetongzhiChart2 = echarts.init(document.getElementById("zuoyetongzhiChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                zuoyetongzhiChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zuoyetongzhiChart2.resize();
                };
            }
        });
      })
